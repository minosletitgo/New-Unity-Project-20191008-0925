...
public class SpriteSelector : ScriptableWizard
{
	...
	void OnGUI ()
	{
		...
		else
		{
            ...
            GUILayout.Label(
                string.Format("Name:{0}", atlas.name),
                "LODLevelNotifyText"
                );
            GUILayout.Label(
                string.Format("Size:{0}*{1}", atlas.spriteMaterial.mainTexture.width, atlas.spriteMaterial.mainTexture.height),
                "LODLevelNotifyText"
                );
            GUILayout.Label(
                string.Format("Occupancy:{0}%", (atlas.Occupancy() * 100).ToString("0.0")),
                "LODLevelNotifyText"
                );
					...
					for (; offset < sprites.size; ++offset)
					{
						...
						if (GUI.Button(rect, ""))
						{
							...
							else
							{
                                ...
                                NGUIContextMenu.AddItem("FindReference", false, FindSpriteReference, sprite);
    ...
    void FindSpriteReference(object obj)
    {
        if (this == null) return;
        if (NGUISettings.atlas == null) return;
        UISpriteData sd = obj as UISpriteData;
        FindAtlasSpriteReference.CreateWindow(NGUISettings.atlas, sd.name);
    }
