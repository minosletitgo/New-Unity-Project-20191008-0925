...
public class UIAtlasInspector : Editor
{
		...
		if (mAtlas.spriteMaterial != null)
		{
			...
			if (sprite != null)
			{
				...
				if (tex != null)
				{
                    ...
                    if (GUILayout.Button("Save All As..."))
                    {
                        string strFolderPath = EditorUtility.OpenFolderPanel("Save To Where", "Assets/Resources/UI", "");
                        if (Directory.Exists(strFolderPath))
                        {
                            BetterList<string> sprites = mAtlas.GetListOfSprites(NGUISettings.partialSprite);

                            foreach (string spr in sprites)
                            {
                                if (!string.IsNullOrEmpty(spr))
                                {
                                    string strPathDetail = strFolderPath + "/" + spr + ".png";
                                    NGUISettings.currentPath = System.IO.Path.GetDirectoryName(strPathDetail);

                                    UIAtlasMaker.SpriteEntry se = UIAtlasMaker.ExtractSprite(mAtlas, spr);

                                    if (se != null)
                                    {
                                        byte[] bytes = se.tex.EncodeToPNG();
                                        File.WriteAllBytes(strPathDetail, bytes);
                                        //AssetDatabase.ImportAsset(path);
                                        if (se.temporaryTexture) DestroyImmediate(se.tex);
                                    }
                                }
	...
	public override void OnPreviewGUI (Rect rect, GUIStyle background)
	{
        ...
        NGUIEditorTools.DrawSpriteAndViewInAtlas(rect, mAtlas, NGUISettings.selectedSprite);
    }
