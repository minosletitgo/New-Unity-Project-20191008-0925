...
{
    static FindAtlasReference m_Window;
    ...
    [MenuItem("Tool/NGUI/查找NGUI图集的引用", false, 1001)]
    static void CreateWindow()
    {
        m_Window = (FindAtlasReference)EditorWindow.GetWindow(typeof(FindAtlasReference), false, "查找NGUI图集的引用");
        m_Window.Show();
    }
    ...
    void DoCollectSprite()
    {
        ...
        for (int iTrans = 0; iTrans < m_arySelectingTrans.Length; iTrans++)
        {
            ...
            foreach (UISprite _spr in arySprRet)
            {
                ...
                if (_spr != null && _spr.atlas == m_AtlasOnlyOne)
                {
